...
const getResponse = async (slug: string, params: Record<string, any>={}, key: string='') => {
  try {
    ...
    throw new Error(
      `YouTube API 요청 중 오류가 발생했습니다: ${errorMessage}`
    );
  }
};
...
const getAllResponses = async (slug: string, params: Record<string, any>={}, key: string='', maxItems = Infinity) => {
  let results: any[] = [];
  let nextPageToken = null;

  do {
    const data: YouTubeApiResponse = await getResponse(slug, {
      ...params,
